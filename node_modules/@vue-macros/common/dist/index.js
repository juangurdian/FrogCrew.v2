...
var require2;
function getRequire() {
  if (require2) return require2;
  try {
    if (globalThis.process?.getBuiltinModule) {
      const module = process.getBuiltinModule("module");
      if (module?.createRequire) {
        return require2 = module.createRequire(import.meta.url);
      }
    }
  } catch {
  }
...
function detectVueVersion(root, defaultVersion = 3.5) {
  const require3 = getRequire();
  if (!require3) {
    console.warn(`Cannot detect Vue version. Default to Vue ${defaultVersion}`);
    return defaultVersion;
  }
  const { resolve } = require3("node:path");
  const { statSync } = require3("node:fs");
  const { getPackageInfoSync } = require3("local-pkg");
  root ||= process.cwd();
  let isFile = false;
  try {
...
var VUE_PLUGINS = ["vite:vue", "unplugin-vue"];
...
export {
  ...
  createFilter,
  createFilter2 as createRollupFilter,
  detectVueVersion,
  getFileCodeAndLang,
  getFilterPattern,
  getVuePluginApi,
